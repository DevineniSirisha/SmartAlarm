package com.example.s531505.smartalarm;

public class StepDetector {

    private static final int ACCELARATOR_RING_SIZE = 50;
    private static final int VEL_RING_SIZE = 10;

    // change this threshold according to your sensitivity preferences
    private static final float STEP_THRESHOLD = 50f;

    private static final int STEP_DELAY = 250000000;

    private int accelRingCounter = 0;
    private float[] accelRingX = new float[ACCELARATOR_RING_SIZE];
    private float[] accelRingY = new float[ACCELARATOR_RING_SIZE];
    private float[] accelRingZ = new float[ACCELARATOR_RING_SIZE];
    private int velRingCounter = 0;
    private float[] velRing = new float[VEL_RING_SIZE];
    private long lastStepTime = 0;
    private float oldVelocityEstimate = 0;

    private StepCounter listener;

    public void registerListener(StepCounter listener) {
        this.listener = listener;
    }


    public void updateAccel(long timeNs, float x, float y, float z) {
        float[] currentAccel = new float[3];
        currentAccel[0] = x;
        currentAccel[1] = y;
        currentAccel[2] = z;

        // First step is to update our guess of where the global z vector is.
        accelRingCounter++;
        accelRingX[accelRingCounter % ACCELARATOR_RING_SIZE] = currentAccel[0];
        accelRingY[accelRingCounter % ACCELARATOR_RING_SIZE] = currentAccel[1];
        accelRingZ[accelRingCounter % ACCELARATOR_RING_SIZE] = currentAccel[2];

        float[] worldZ = new float[3];
        worldZ[0] = SensorFilter.sum(accelRingX) / Math.min(accelRingCounter, ACCELARATOR_RING_SIZE);
        worldZ[1] = SensorFilter.sum(accelRingY) / Math.min(accelRingCounter, ACCELARATOR_RING_SIZE);
        worldZ[2] = SensorFilter.sum(accelRingZ) / Math.min(accelRingCounter, ACCELARATOR_RING_SIZE);

        float normalization_factor = SensorFilter.norm(worldZ);

        worldZ[0] = worldZ[0] / normalization_factor;
        worldZ[1] = worldZ[1] / normalization_factor;
        worldZ[2] = worldZ[2] / normalization_factor;

        float currentZ = SensorFilter.dot(worldZ, currentAccel) - normalization_factor;
        velRingCounter++;
        velRing[velRingCounter % VEL_RING_SIZE] = currentZ;

        float velocityEstimate = SensorFilter.sum(velRing);

        if (velocityEstimate > STEP_THRESHOLD && oldVelocityEstimate <= STEP_THRESHOLD
                && (timeNs - lastStepTime > STEP_DELAY)) {
            listener.step(timeNs);
            lastStepTime = timeNs;
        }
        oldVelocityEstimate = velocityEstimate;
    }
}
